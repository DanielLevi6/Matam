...
Event createEvent(char* event_name,int event_id,Date date){
    ...
    event->event_name = malloc(sizeof(strlen(event_name))+sizeof(char));
...
PQElement copyEvent(PQElement event){
    ...
    Event copy_event=malloc(sizeof(*copy_event));
    copy_event=createEvent(((Event)event)->event_name,((Event)event)->event_id,((Event)event)->date);
...
void subAllEventMembers(EventManager em,Event event){
    ...
    while(iterator){
        emRemoveMemberFromEvent(em,getMemberID(iterator),event->event_id);
...
EventManagerResult addMemberToEvent(Event event,char* member_name,int involved_events,int member_id){
    ...
    PriorityQueueResult result=pqInsert(event->staff,member,&member_id);
    freeMember(member);
...
EventManagerResult removeMemberFromEvent(Event event,int member_id){
    ...
    Member member=getMember(event->staff,member_id);
    if(!member){
...
void printEventMembersList(Event event,const char* file_name){
    if(!event){
        return;
    }
    Member iterator=pqGetFirst(event->staff);
    ...
    printMemberName(iterator,file_name);
    while(iterator){
        iterator=pqGetNext(event->staff);
        fprintf(fp,",");
        printMemberName(iterator,file_name);
