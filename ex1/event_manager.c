...
#define ERROR -1
//טעות הגיונית-פונקציות ההשוואה של member,event
...
EventManager createEventManager(Date date){
    ...
    em->events_list=pqCreate(copyEvent,freeEvent,compareEvents,dateCopy,dateDestroy,dateCompare);
...
EventManagerResult emAddEventByDate(EventManager em, char* event_name, Date date, int event_id){
    ...
    if (iterator!=NULL&&getEventID(iterator)==event_id)
    {
...
EventManagerResult emRemoveEvent(EventManager em, int event_id){
    ...
    pqRemoveElement(em->events_list,the_event);
    return EM_SUCCESS;}
/*    Event iterator=pqGetFirst(em->events_list);
    if (getEventID(iterator)==event_id)
    {
...
EventManagerResult emChangeEventDate(EventManager em, int event_id, Date new_date){
    ...
    char* event_name=getEventName(event);
...
/*
 ...
 /*   Event iterator=pqGetFirst(em->events_list);
    while(iterator){
        if(strcmp(getEventName(iterator),getEventName(event))==0&&dateCompare(getEventDate(iterator),new_date)==0&&getEventID(iterator)!=event_id){
            return EM_EVENT_ALREADY_EXISTS;
        }
        iterator=pqGetNext(em->events_list);
    }
    pqChangePriority(em->events_list,event,getEventDate(event),new_date);
...
EventManagerResult emAddMemberToEvent(EventManager em, int member_id, int event_id){
    ...
    EventManagerResult result=addMemberToEvent(event,getMemberName(member),getInvolvedEvents(member),getMemberID(member));
...
void emPrintAllEvents(EventManager em, const char* file_name){
    ...
    Event iterator=pqGetFirst(em->events_list);
    printEventMembersList(iterator,file_name);
    while(iterator){
        iterator=pqGetNext(em->events_list);
        printEventMembersList(iterator,file_name);
    }
