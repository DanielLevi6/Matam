...
Member createMember(char* member_name,int member_id,int event_counter){
    ...
    member->member_name=malloc(sizeof(strlen(member_name))+1);
...
PQElement copyMember(PQElement member) 
{
    ...
    new_member=malloc(sizeof(strlen(((Member)member)->member_name))+1);
...
void printMemberName(Member member,const char* file_name){
    if(!member){
        return;
    }
...
PQElementPriority copyMemberPriority(PQElementPriority member_priority) 
{
    ...
    new_member_priority = (PriorityMember)member_priority;

    return new_member_priority;
}
...
{
    int first_event_counter = ((PriorityMember)first_member_priority)->event_counter;
    int first_member_id = ((PriorityMember)first_member_priority)->member_id;
    int second_member_counter = ((PriorityMember)second_member_priority)->event_counter;
    int second_member_id = ((PriorityMember)second_member_priority)->member_id;

    if (first_event_counter != second_member_counter) {
        return first_event_counter - second_member_counter;
    }
    return second_member_id - first_member_id;
}
/**************************************/
